...
Das REST-API wurde mit der Bibliothek \mintinline{latex}{Flask} \ref{g3_flask} entwickelt und besteht aus einer POST-Schnittstelle, welche auf dem von der HTW bereitgestelltem Server unter \textit{/notify} erreichbar ist. 

Das API wurde unter Zuhilfenahme von \mintinline{latex}{Flask.Blueprints} \ref{g3_flaskbp} entwickelt und wird von einem \mintinline{latex}{Waitress}-Webserver \ref{g3_waitress} bereitgestellt. 
...
Sobald neue Sitzungen importiert und verarbeitet wurden, werden diese zunächst mit dem MongoDB-Treiber \mintinline{latex}{PyMongo} \ref{g3_mongodb} in die Datenbank geschrieben. 
...
Der Zugriff auf OdeNet geschieht dabei mit der python Bibliothek \mintinline{latex}{WN} \ref{g3_wn}. 
...
\label{g3textv}
Für die Textverarbeitung nutzt der Code dieser Arbeit vollständig die Funktionalitäten und Strukturen von \mintinline{latex}{spaCy} \ref{g3_spacy}. 
...
Angemerkt sei an dieser Stelle, dass es nicht möglich war, alle Regeln aus \ref{g3_wieg} zu implementieren, da der von den Forschern genutzte Dependency-Parser umfangreichere Ergebnisse liefert, als jener von \mintinline{latex}{spaCy}. 

Eine Liste mit Negationsworten und dem jeweiligen Negationstyp wurde \ref{g3_polcla} entnommen und ist ebenfalls über die Klasse \mintinline{latex}{Lexicon} zugreifbar. 
...
Aus diesem Grund wird eine Liste mit verstärkenden Gradpartikeln, welche ebenfalls aus \ref{g3_polcla} bezogen werden, eingelesen und über die \mintinline{latex}{Lexicon} Klasse bereitgestellt. 
