...
\todo{nicht funktionale? ansonsten oben aus dem text nehmen}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{images/03-cme/Project-Modules.pdf}
    \end{center}
...
zusätzlich konfigurierten nginx-Reverse-Proxy. Dieser hat neben ssh \todo{glos} den
...
im vorherigen Abschnitt beschrieben, \todo{der text muss noch geschrieben
werden} das \textit{expose}-Argument genutzt, um den MongoDB-Container nur
...
Der Server ist nur über das \gls{htw}-Netzwerk erreichbar. Zusätzlich ist die API mit
